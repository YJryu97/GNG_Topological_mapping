...
class GrowingNeuralGas:
    ...
    def fit(self, dataset : np.ndarray,
        ...
        for iteration in range(1, iterations + 1): 
            ...
            #if (iteration % 1000 == 0) and (self.n_vertex < max_vertex):
             #   vertices = self.graph.get_vertices()
              #  mean_squared_error = []
              #  total_error = 0
               # for key in vertices:
               #     total_error += self.graph.get_vertex_param(key=key, param_key='error')
                #mse = total_error / self.n_vertex    
                #array = np.append(mean_squared_error, [iteration, mse])
                #np.savetxt("C:\GNG2\error\mean_error.csv",array, delimiter=",")
