...
class Graph:
    ...
    def get_edges(self, key):
        ...
        edges_out = list(self.vertices[key].edges_out.keys()) # gets the vertex that goes out from the specific vertex
        edges_in = list(self.vertices[key].edges_in.keys())  # gets the vertex that goes in to the specific vertex
        return {'in' : edges_in, 'out' : edges_out} #'in' key , edges_in value
